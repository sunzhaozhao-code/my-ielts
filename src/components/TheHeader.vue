...
function isActive(link: string) {
  if (link === '/')
    return route.path === '/'
  if (link === '/resources')
    return ['/resources', '/vocabulary', '/grammar', '/listening', '/reading', '/writing', '/speaking'].some(path => route.path.startsWith(path))
        ...
        <div class="flex items-center justify-start">
          ...
          <div class="hidden w-full items-center justify-between lg:order-1 lg:w-auto lg:flex">
            <ul class="mt-4 flex flex-col items-center justify-items-center text-center align-middle text-sm font-medium lg:mt-0 lg:flex-row space-x-6 xl:space-x-8">
              ...
              >
                <router-link
                  :class="isActive(m.link) ? 'block flex flex-row items-center rounded text-primary-700 dark:text-primary-500' : 'block flex flex-row items-center text-gray-700 dark:text-gray-400 hover:text-primary-700 dark:hover:text-white'"
                  :to="m.link"
                >
